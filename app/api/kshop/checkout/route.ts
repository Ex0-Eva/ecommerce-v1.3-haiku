...
import { createKShopPayment } from "@/lib/kshop";
...
export async function POST(req: Request) {
  try {
    ...
    const payment = await createKShopPayment({
      amount: total,
      description: `Order #${order.id}`,
      orderId: order.id,
      customerEmail: session?.user?.email ?? undefined,
      callbackUrl: `${origin}/api/kshop/webhook`,
    ...
    console.error("KShop checkout error:", error);
    return NextResponse.json(
