...
/** Returns active variants sorted by sort_order */
...
export default function ProductsPage() {
  ...
  // Map of "productId" or "productId:variantId" → selected variant
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        ...
        const message = err instanceof Error ? err.message : "ไม่สามารถโหลดสินค้าได้";
  ...
  const handleAddToCart = (product: Product) => {
    ...
    if (hasVariants && !selectedVariant) return; // guard — button should already be disabled
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {loading
            ...
            : products.map((product) => {
                ...
                // Effective price: selected variant price, or min variant price, or product price
                    ...
                    <div className="p-5">
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <div>
                          ...
                          {/* Stock hint — show for selected variant or non-variant product */}
                          {(() => {
                        ...
                        {isAffiliateProduct ? (
                          ...
                          >
                            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2M12 20a8 8 0 110-16 8 8 0 010 16z" />
                            </svg>
                            ซื้อจาก Marketplace
                          </a>
                        ) : isOutOfStock ? (
                          ...
                          <button
                            type="button"
                            disabled
                            className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-400 cursor-not-allowed"
                          >
                            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-semibold text-white transition active:scale-95 ${
        ...
        {error && (
          <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
            <p className="text-lg font-semibold text-red-700">⚠️ เกิดข้อผิดพลาด</p>
